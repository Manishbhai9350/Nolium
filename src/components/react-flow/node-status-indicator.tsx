...
export const SpinnerLoadingIndicator = ({
  ...
  return (
    <div className="relative">
      <StatusBorder className="border-blue-800/40">{children}</StatusBorder>
...
export const NodeStatusIndicator = ({
  ...
  switch (status) {
    ...
    case "success":
      return (
        <StatusBorder className={cn("border-emerald-700/70",className)}>{children}</StatusBorder>
      );
    case "error":
      return <StatusBorder className={cn("border-red-700/70",className)}>{children}</StatusBorder>;
    default:
      return <>{children}</>;
  }
};
